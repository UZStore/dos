...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.items = GetResourceState('uz_TrygonInventory') ~= 'missing' and exports['uz_TrygonInventory']:LoadInventory(PlayerData.source) or {}
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    function self.Functions.AddMoney(moneytype, amount, reason)
        ...
        if moneytype == 'cash' then exports['uz_TrygonInventory']:AddItem(self.PlayerData.source, 'cash', amount) end
    ...
    function self.Functions.RemoveMoney(moneytype, amount, reason)
        ...
        if moneytype == 'cash' then
            if amount < 0 then return end
            self.PlayerData.money[moneytype] = self.Functions.GetMoney('cash')
            if (self.PlayerData.money[moneytype] - amount) < 0 then return false end
            self.PlayerData.money[moneytype] = self.PlayerData.money[moneytype] - amount
            return exports['uz_TrygonInventory']:RemoveItem(self.PlayerData.source, 'cash', amount)
        end
        if not self.PlayerData.money[moneytype] then return false end
    ...
    function self.Functions.GetMoney(moneytype)
        ...
        moneytype = moneytype:lower()
        if moneytype == "cash" then
            local GetItems = exports['uz_TrygonInventory']:LoadInventory(self.PlayerData.source)
            if GetItems then
                for index, item in pairs(GetItems) do
                    if item.ItemData ~= nil or item.ItemData?.Name == 'cash' then
                        self.PlayerData.money['cash'] = item.ItemData?.Amount or 0
...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
        ...
        if GetResourceState('uz_TrygonInventory') ~= 'missing' then exports['uz_TrygonInventory']:SaveInventory('Players') end
    ...
    if license == result then
        ...
        MySQL.transaction(queries, function(result2)
            if result2 then
                ...
                exports['uz_TrygonInventory']:DeleteCharacter(Player.PlayerData.source)
            end
        end)
    else
        DropPlayer(source, Lang:t("info.exploit_dropped"))
